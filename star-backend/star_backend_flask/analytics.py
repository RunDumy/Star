...
def get_analytics(current_user):
    ...
    try:
        ...
        active_users_response = supabase.table('profiles').select('id', count='exact').gte('last_active', active_start.isoformat()).execute()
        active_users = active_users_response.count

        # Get total notifications
        notifications_response = supabase.table('notifications').select('id', count='exact').gte('created_at', start_date.isoformat()).execute()
        total_notifications = notifications_response.count

        # Get zodiac distribution
        zodiac_response = supabase.table('profiles').select('zodiac_sign').execute()
        zodiac_counts = {}
        for profile in zodiac_response.data:
            sign = profile.get('zodiac_sign', 'unknown')
            zodiac_counts[sign] = zodiac_counts.get(sign, 0) + 1

        # Get activity patterns (hourly for last 24 hours)
        activity_patterns = []
        for hour in range(24):
            hour_start = now.replace(hour=hour, minute=0, second=0, microsecond=0)
            hour_end = hour_start + timedelta(hours=1)

            hour_response = supabase.table('notifications').select('id', count='exact').gte('created_at', hour_start.isoformat()).lt('created_at', hour_end.isoformat()).execute()
            activity_patterns.append({
                'hour': hour,
                'count': hour_response.count
            })

        # Get engagement metrics
        likes_response = supabase.table('notifications').select('id', count='exact').eq('type', 'like').gte('created_at', start_date.isoformat()).execute()
        comments_response = supabase.table('notifications').select('id', count='exact').eq('type', 'comment').gte('created_at', start_date.isoformat()).execute()
        follows_response = supabase.table('notifications').select('id', count='exact').eq('type', 'follow').gte('created_at', start_date.isoformat()).execute()
        messages_response = supabase.table('chat_messages').select('id', count='exact').gte('created_at', start_date.isoformat()).execute()

        engagement_metrics = {
            'likes': likes_response.count,
            'comments': comments_response.count,
            'follows': follows_response.count,
            'messages': messages_response.count
        }

        # Get planet button click metrics
        planet_clicks_response = supabase.table('analytics_events').select('event_data').eq('event_type', 'planet_button_click').gte('created_at', start_date.isoformat()).execute()
        planet_clicks = {}
        for event in planet_clicks_response.data:
            planet = event.get('event_data', {}).get('planet', 'unknown')
            planet_clicks[planet] = planet_clicks.get(planet, 0) + 1

        # Get constellation reveal metrics
        constellation_reveals_response = supabase.table('analytics_events').select('event_data').eq('event_type', 'constellation_reveal').gte('created_at', start_date.isoformat()).execute()
        constellation_reveals = {}
        for event in constellation_reveals_response.data:
            zodiac = event.get('event_data', {}).get('zodiac', 'unknown')
            constellation_reveals[zodiac] = constellation_reveals.get(zodiac, 0) + 1

        # Get top zodiac pairs (simplified - just return some mock data for now)
        # In a real implementation, this would analyze compatibility based on interactions
        top_zodiac_pairs = [
            {'pair': 'Leo ♌ & Aries ♈', 'compatibility': 95, 'count': 45},
            {'pair': 'Pisces ♓ & Scorpio ♏', 'compatibility': 92, 'count': 38},
            {'pair': 'Taurus ♉ & Virgo ♍', 'compatibility': 90, 'count': 42},
            {'pair': 'Gemini ♊ & Aquarius ♒', 'compatibility': 88, 'count': 35},
            {'pair': 'Cancer ♋ & Capricorn ♑', 'compatibility': 85, 'count': 40}
        ]
        ...
        analytics_data = {
            ...
            'zodiacDistribution': zodiac_counts,
            'activityPatterns': activity_patterns,
            'engagementMetrics': engagement_metrics,
            'planetClicks': planet_clicks,
            'constellationReveals': constellation_reveals,
            'topZodiacPairs': top_zodiac_pairs
        }
